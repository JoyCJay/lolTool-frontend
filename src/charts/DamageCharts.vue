<template>
    <v-layout d-flex align-space-around justify-center row fill-height wrap>
      <v-flex xs12 sm4 md4>
        <div id="damageChart" :style="{width: '500px', height: '300px'}">

        </div>
      </v-flex>
      <v-flex xs12 sm4 md4>
        <div id="damageChart2" :style="{width: '500px', height: '300px'}">
  ...
  export default {
    ...
    methods: {
      drawChart(match){
        ...
        let pieChart = this.$echarts.init(document.getElementById('damageChart'));
        let pieChart2 = this.$echarts.init(document.getElementById('damageChart2'));
        let option = {
